page-feed
  ...
  #page-feed
    .container.no-padding-s
      h4.center(if='{ title }') { title }
      .row
        .col.s12.m6.l4(each='{ pin in pins }')
          .card.hover-card
            a.card-image(href='#pins/{ pin.id }{ pin.mock ? "?mock=1" : "" }', style='background-image: url({ pin.photos && pin.photos.length > 0 ? pin.photos[0] : util.site_url("/public/image/pin_photo.png") });')
            .card-content
              .card-description
                .card-author
